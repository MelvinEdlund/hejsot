...
export function BgThemePreview({ theme, active }: { theme: BgTheme; active: boolean }) {
  ...
  const colors: Record<BgTheme, string> = {
    ...
    starfield: "from-[#0d0c17] to-[#1a1030]",
    bubbles: "from-[#1a0d2e] to-[#0d1a2e]",
    aurora: "from-[#ff7ca1]/20 via-[#b28aff]/20 to-[#7cd4ff]/20",
    roses: "from-[#2e0d1a] to-[#1a0d0d]",
    heartbeat: "from-[#2e0d1a] to-[#1a0d20]",
    snow: "from-[#0d1a2e] to-[#0d1020]",
  };
  ...
  return (
    <button
      type="button"
      className={[
        "relative flex flex-col items-center justify-center gap-1.5 overflow-hidden rounded-xl border p-3 transition-all",
        "bg-gradient-to-br",
        colors[theme],
        active
    ...
    >
      <span className="text-[18px] leading-none">{emojis[theme]}</span>
      <span className="text-[11px] text-fg/70">{labels[theme]}</span>
    </button>
  );
}
